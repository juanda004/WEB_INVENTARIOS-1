...
/**
 * Aplica o revierte el ajuste de inventario buscando el producto por código en TODAS las tablas.
 * CORRECCIÓN: Usa parámetros nombrados únicos para evitar el error HY093.
 * * @param array $productos Array de productos desde el JSON (debe contener 'codigo' y 'cantidad').
 ...
 */
function gestionarInventario(PDO $pdo, array $productos, string $operacion) {
    ...
    foreach ($productos as $producto) {
        ...
        foreach ($tablas as $tabla) {
            ...
            // 1. Construir la consulta de actualización con parámetros únicos
            $sql_update = "UPDATE `$tabla` 
                           SET CANT = CANT {$operacion} :cantidad_update 
                           WHERE CODIGO = :codigo"; 
            
            $params = [
                ':cantidad_update' => $cantidad, // Parámetro para el SET
                ':codigo' => $codigo
            ];
            
            // Si es un descuento ('-'), incluimos la condición de stock mínimo.
            if ($operacion === '-') {
                // Usamos un parámetro diferente para la condición WHERE (soluciona el error HY093)
        ...
        if (!$encontrado_y_actualizado) {
            if ($operacion === '-') {
                 ...
                 // Es una reversión que falló, se ignora el producto faltante pero se continúa.
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['accion_solicitud'])) {
    ...
    try {
        ...
        if ($nuevo_estado === 'entregado' && $estado_actual !== 'entregado') {
            gestionarInventario($pdo, $productos, '-'); // Descontar unidades
        ...
        // Caso B: Transición de ENTREGADO a otro estado (Reversión/Devolución)
            ...
            gestionarInventario($pdo, $productos, '+'); // Revertir unidades
                    ...
                    <tr>
                        ...
                        <td>
                            <form action="ver_solicitudes.php" method="POST" class="d-inline">
                                ...
                                <a href="descargar_solicitudes_pdf.php?id=<?php echo htmlspecialchars($s['id']); ?>" class="btn btn-secondary btn-sm">
                                    Descargar PDF
                                </a>
                            </form>
                        </td>
